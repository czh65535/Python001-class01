...
class Maoyan2Spider(scrapy.Spider):
    ...
    def start_requests(self):
            target_url = f'https://maoyan.com/films?showType=3&offset=0'
    ...
    def parse(self, response):
        ...
        for tags in Selector(response=response).xpath('//div[@class="channel-detail movie-item-title"]'):
            ...
            item = SpirdersItem()
            movie_name = ''.join(tags.xpath('./a/text()').extract_first())
            movie_url = self.origin_url + ''.join(tags.xpath('./a/@href').extract_first())
    ...
    def parse2(self, response):
        ...
        for tags in Selector(response=response).xpath('//div[@class="movie-brief-container"]'):
            movie_type = ','.join(tags.xpath('./ul/li/a/text()').extract()).replace(' ', '')
            release_time = ''.join(tags.xpath('./ul/li[last()]/text()').extract())
